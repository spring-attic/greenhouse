...
public class JdbcEventRepository implements EventRepository {
	...
	public List<Event> findUpcomingEvents(DateTime clientTime) {
		return jdbcTemplate.query(SELECT_UPCOMING_EVENTS, eventMapper, clientTime.withZone(DateTimeZone.UTC).toDate());
	}
	...
	private RowMapper<Event> eventMapper = new RowMapper<Event>() {
		public Event mapRow(ResultSet rs, int row) throws SQLException {
			return new Event(rs.getLong("id"), rs.getString("title"), new DateTime(rs.getTimestamp("startTime"), DateTimeZone.UTC), new DateTime(rs.getTimestamp("endTime"), DateTimeZone.UTC),
					rs.getString("slug"), rs.getString("description"), rs.getString("hashtag"), rs.getString("groupName"), rs.getString("groupSlug"));
		}
	};
	...
	private static final String SELECT_UPCOMING_EVENTS = "select e.id, e.title, e.startTime, e.endTime, e.slug, e.description, g.hashtag, g.name as groupName, g.slug as groupSlug from Event e " + 
		"inner join MemberGroup g on e.memberGroup = g.id where e.endTime > ? order by e.startTime";

	private static final String SELECT_EVENT_BY_SLUG = "select e.id, e.title, e.startTime, e.endTime, e.slug, e.description, g.hashtag, g.name as groupName, g.slug as groupSlug from Event e " + 
